...
Page({
  ...
  xieyi: function () {
    wx.showModal({
      title: '圣豆物联协议',
      showCancel:false,
      content: '欢迎您使用智能井盖小程序。您可使用本小程序以实现智能设备的控制、共享等功能。我们将严格遵守相关法律法规和隐私政策以保护您的个人信息。为提供基本服务，需要联网以及调用您的如下权限或功能，以收集必要的个人信息:必要权限(用于本小程序的基本服务和功能)帐号信息(必选)用于本小程序内使用米家帐号快速登录的功能',
      success (res) {
        if (res.confirm) {
          console.log('用户点击确定')
        } else if (res.cancel) {
  ...
  formSubmit: function (e) {
    ...
    console.log("按了");
    wx.request({
      url: "https://" + app.globalData.MainURL + "/auth/smsLog",
      method: "POST",
      data: {
